...
interface VolumeControlOverlayProps {
  ...
  onVolumeChange?: (volume: number) => void; // Add a callback for volume changes
}

const VolumeControlOverlay: React.FC<VolumeControlOverlayProps> = ({ position, rotation, style, onVolumeChange }) => {
  ...
  return (
    <Html position={[position.x, position.y, position.z]} rotation={rotation} style={style} transform>
      <div
        className="bg-white bg-opacity-75 rounded-lg p-4 shadow-xl flex flex-col items-center justify-between select-none"
        style={{ userSelect: 'none' }} // Ensure the text is not selectable
      >
        <div>
          <button
