...
export function RotavapRemoveBumpTrap(props: GroupProps) {
  const { scene, animations } = useGLTF("./17-remove bump trap.glb");
  const clonedScene = scene.clone(); // Clone the scene for isolated use
  const rb_flask = clonedScene.children[2]!.children[3];
  console.log(clonedScene); // Hide everything but this <=====
  ...
  const hitboxPosition: Vector3 = [0, 4.6, 1.5]; // Replace x, y, z with the coordinates near the power button
  const hitboxScale: Vector3 = [.7, .6, .7]; // Replace width, height, depth with appropriate dimensions
...
useGLTF.preload("./17-remove bump trap.glb");
