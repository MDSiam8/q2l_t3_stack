...
export const RB_Flask_TA = React.forwardRef<any, RBFlaskTAProps>((props, ref) => {
  const FlaskMesh = useGLTF("./6-NoMachine_RBFlask_WA.glb") as any;
  const { scene, animations } = FlaskMesh;
  
  const clonedScene = scene.clone(); // Clone the scene for isolated use
  const { actions } = useAnimations(animations, clonedScene);

  const initialPosition = new THREE.Vector3(0, 5, -3); // Initial position
