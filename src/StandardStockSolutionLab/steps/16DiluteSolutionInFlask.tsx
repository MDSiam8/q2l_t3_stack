...
const Step16DiluteSolutionInFlask = forwardRef<THREE.Group, Step16Props>(
  ({ nextButtonRef }, ref) => {
    ...
    // Function to handle distilled water animation when clicked
    const handleDistilledWaterClick = () => {
      if (distilledWaterRef.current) {
        const targetPosition = new THREE.Vector3(0, 6.4, 1.2);
        const tl = gsap.timeline();
    ...
    // Function to start filling water by animating through water levels
    const startFillingWater = () => {
      pourInterval.current = setInterval(() => {
        setWaterLevel((prevLevel) => {
          const newLevel = prevLevel + 1;
          // Check the water level to see if it reached meniscus or overfilled 
           if (newLevel === 4) {
            alert("Success! Water has reached the meniscus. Move to the next step.");
            setNextEnabled(nextButtonRef); // Enable the next step
            clearInterval(pourInterval.current!); // Stop further filling
          } else if (newLevel > 4) {
            alert("Overfilled! Try again.");
            resetWater(); // Reset if overfilled
            clearInterval(pourInterval.current!);
          }

          // Update the animation based on new water level
          if (flaskFillRef.current) {
            flaskFillRef.current.playAnimationAtFrame(newLevel); 
          }
          return newLevel;
        });
      }, 50); // Adjust the fill speed as needed
    };

    // Function to stop filling water manually
    const stopFillingWater = () => {
      if (pourInterval.current) {
        clearInterval(pourInterval.current);
      }
    ...
    // Reset function to clear water and reset animation
    const resetWater = () => {
      setWaterLevel(0); // Reset water level state
      if (flaskFillRef.current) {
        flaskFillRef.current.playAnimationAtFrame(0); // Reset the animation to empty
      }
      setNextDisabled(nextButtonRef); // Disable the next button
    };
    ...
    return (
      <group>
        ...
        >
          <DistilledWater /> {/* Distilled water model */}
        </group>
        ...
        <Html position={[0, 8, 0]} transform scale={0.5} rotation-y={Math.PI / 2}>
          <button
            onMouseDown={startFillingWater}
            onMouseUp={stopFillingWater}
            className="rounded-md p-3 text-sm font-bold text-white bg-blue-500 hover:bg-blue-600 shadow-lg transition-transform duration-300"
